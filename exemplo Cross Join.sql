...
/*
 * CROSS JOIN é um tipo especial de junção em que cada linha de uma tabela é combinada 
 * com todas as linhas de outra tabela. Isso resulta em um produto cartesiano engtre as duas tabelas,
 * ou seja, todas s combinaçoes possiveis de linhas são produzidas.
 * Por exemplo: temos duas tabelas, uma chamada cores e outra chamada frutas, e queremos encontrar 
 * todas as combinaçoes possiveis de cores e fruta.
 */
...
*onde há uma correspondencia entre o id na tabela clientes e o cliente_id na tabwla pedidos*/
...
/*RIGTH join: Retorna todos os registros da tabela da direita  (segunda tabela mencionada) e os registros
correspondentes da tabela da esquerda(primeira tabela mencionada).*/

select c.id, c.nome, p.id, p. data_pedido p.cliente_id
from clientes c 
rigth join pedidos p on c.id = p.cliente_id;

/* este comando retorna todos os registros da tabela pedidos e os registros correspondentes da tabela clientes, se houver, com base na correspondencia
de id e cliente_id*/